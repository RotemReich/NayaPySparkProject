...
@dag(
    dag_id = "pyspark_project_pipeline",
...
def pyspark_project_pipeline():
    ...
    def models_and_colors_create():
        run_command("models_and_colors.py", detached=False, minio=True)
    
    @task
    def cars_create():
        run_command("cars.py", detached=False, minio=True)
    ...
    @task
    def create_topic_alert_data():
        create_kafka_topic("alert-data")
    
    @task
    def alerting_detection():
        run_command("alerting_detection.py", detached=True)

    @task
    def wait_for_message_alerts():
        wait_for_message("alert-data")
    ...
    (
        ...
        create_topic_alert_data() >>
        alerting_detection() >>
        wait_for_message_alerts()
    )    
